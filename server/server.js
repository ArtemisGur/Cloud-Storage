...
APP.post('/uploadNewFiles', (req, res) => {
    ...
    dirname = `${__dirname}/files/${req.body.path}/${req.body.fileName}`
    file.mv(`${__dirname}/files/${req.body.path}/${req.body.fileName}`,
        function (err) {
            if (err) {
                res.status(500).send({ msg: "Error occurred" })
            }
            let stat = fs.statSync(`${__dirname}/files/${req.body.path}/${req.body.fileName}`)
...
APP.post('/createDir', (req, res) => {
    ...
    dirname = `${__dirname}/files/${req.body.path}/${req.body.name}`
